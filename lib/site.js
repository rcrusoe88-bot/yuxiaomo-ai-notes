...
export const site = {
  ...
  wechat: {
    name: '信使引擎'
    // 不放公众号后台链接：带 token 的 URL 既泄露会话参数、对访客也不可用。
  },
...
// MDX 正文里手写的根相对路径（图片等）在 GitHub Pages 子路径下不会被
// Next 自动补 basePath，构建前统一重写，避免线上 404。
// 同时覆盖 markdown 图片 `![alt](/x.png)` 与 JSX `<img src="/x.png">` 两种写法。
export const withBasePaths = source =>
  source
    .replace(/(!\[[^\]]*\]\()((?:\/)(?!\/)[^)]*\))/g, `$1${pageBase()}$2`)
    .replace(/(src=")((?:\/)(?!\/)[^"]*")/g, `$1${pageBase()}$2`);
