...
export default async function ArticlePage({ params }) {
  ...
  return <main className="article-page"><SiteHeader /><Link className="back-link" href={`/notes/${note.category}`}>← 回到笔记列表</Link><p className="article-meta">{categories[note.category].name.toUpperCase()} / {note.date} / {note.readingTime} MIN READ</p><h1>{note.title}</h1><p className="article-lede">{note.summary}</p><article className="prose">{content}</article></main>;
}
